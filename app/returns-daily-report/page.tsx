...
export default function ReturnsDailyReportPage() {
    ...
    const fetchRows = useCallback(async () => {
        ...
        try {
            ...
            const res = await axios.get(endpoint, { params: { client } });
    ...
    const grouped = useMemo(() => {
        ...
        filteredRows.forEach(row => {
            const date = String(row[dateField] || "Không xác định");
    ...
    const openEdit = (row: ReturnReportRow) => {
        ...
        const next: EditFormState = {
            date: toInputDate(String(row[dateField] || "")),
            note: row.note || "",
            sizeValues: {},
        };
    ...
    const saveEdit = async () => {
        if (!editRow) return;
        const dateField = reportType === "received" ? "received_date" : "shipped_date";
        const totalField = reportType === "received" ? "total_received" : "total_shipped";
        const payload: Record<string, string | number | null> = {
            [dateField]: editForm.date || null,
        ...
        try {
            await axios.put(`/api/returns/${reportType}?id=${editRow.id}&type=${reportType}`, payload);
            setSnackbar({ open: true, message: "Đã cập nhật thành công.", severity: "success" });
            setEditRow(null);
            await fetchRows();
        } catch (error) {
            console.error("[API] Update return error:", error);
    ...
    const handleDelete = async (row: ReturnReportRow) => {
        ...
        try {
            await axios.delete(`/api/returns/${reportType}?id=${row.id}&type=${reportType}`);
            setSnackbar({ open: true, message: "Đã xóa báo cáo.", severity: "success" });
            await fetchRows();
        } catch (err) {
            console.error("[API] Delete return error:", err);
    ...
    const exportPDF = async () => {
        ...
        const title = reportType === "received" ? "BÁO BIỂU NHẬN HÀNG TRẢ SỬA" : "BÁO BIỂU GIAO HÀNG TRẢ SỬA";
        let currentY = 40;

        doc.setFont("Roboto", "bold");
        doc.setFontSize(16);
        doc.text(title, 30, currentY);
        currentY += 20;
        ...
        for (let groupIndex = 0; groupIndex < grouped.length; groupIndex++) {
            ...
            doc.text(`NGÀY: ${group.date}`, 30, currentY);
            ...
            const head = [[
                "STT", "ĐƠN HÀNG", "CRD", "ĐỢT", "MODEL NAME", "SL NGÀY",
                "CÒN LẠI", "ĐƠN VỊ", "ART",
            ...
            const body = group.rows.map((row, index) => {
                const dailyQty = reportType === "received" ? row.total_received : row.total_shipped;
        ...
        doc.save(`${title.replace(/\s+/g, "_")}_${client.replace(/\s+/g, "_")}_${Date.now()}.pdf`);
    };
        ...
        <div className="mx-auto flex h-[calc(100dvh-80px)] min-h-[calc(100dvh-80px)] w-full flex-col gap-4 overflow-hidden bg-slate-50 p-4 text-[14px] text-slate-800 lg:p-6">
            <div className="flex min-h-0 flex-1 flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="mb-6 flex shrink-0 items-start justify-between">
                    <div>
                        <h1 className="text-lg font-bold uppercase tracking-wide text-slate-800">Báo Biểu Hàng Trả Sửa</h1>
                        <p className="mt-1 text-sm text-slate-400">Báo cáo chi tiết nhận và trả hàng sửa hằng ngày</p>
                    </div>
                    <ToggleButtonGroup
                        value={reportType}
                        exclusive
                        ...
                        sx={{
                            "& .MuiToggleButton-root": {
                                ...
                                "&.Mui-selected": { backgroundColor: "#000", color: "#fff", "&:hover": { backgroundColor: "#333" } },
                            },
                        }}
                    >
                                    ...
                                    {group.rows.map((row, index) => {
                                        ...
                                        return (
                                            <tr key={row.id} className={`${rowBg} group transition-colors hover:bg-blue-50/20`}>
                                                ...
                                                <td className="whitespace-nowrap border-b border-r border-slate-100 px-3 py-2.5 text-center font-bold text-[#0284c7]">{dailyQty || 0}</td>
                    ...
                    <div className="max-h-[90vh] w-full max-w-5xl overflow-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl" onClick={event => event.stopPropagation()}>
                        <div className="mb-6 flex items-center justify-between border-b pb-4">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-slate-500">Cập nhật ngày, ghi chú và số lượng size.</p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                            <label className="block">
                                <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500">{reportType === "received" ? "Ngày nhận hàng" : "Ngày trả hàng"}</span>
